  ...
  <section class="container">
    <el-table
      ...
      <el-table-column
        ...
        width="160">
        <template slot-scope="scope">
          <el-button @click="modifyProject(scope.row)" type="primary" v-scroll-to="{element:'#edit'}" size="small">修改</el-button>
          <el-button @click="deleteProject(scope.row)" type="danger" size="small">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <h3 id="edit" class="edit-title">{{isModify ? '修改' : '创建'}}项目：</h3>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        description: [
          { type: 'string', required: true, message: '请添加项目描述', trigger: 'change' }
        ],
      }
    }
  },
  methods: {
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          Api.addProject(params).then(result => {
    ...
    modifyProject(val) {
      this.isModify = true;
      _.filter(this.projectList, item => {
        if (item._id === val._id) {
          this.ruleForm = item;
    ...
    // 调用删除历程api
    delete(title) {
      Api.deleteProject({title}).then(result => {
        this.$message({
          type: 'success',
          message: '删除成功!'
        });
